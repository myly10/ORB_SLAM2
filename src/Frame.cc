...
//Copy Constructor
	Frame::Frame(const Frame &frame):mpORBvocabulary(frame.mpORBvocabulary),
									 ...
									 mK(frame.mK.clone()), mDistCoef(frame.mDistCoef.clone()), mbf(frame.mbf),
									 mb(frame.mb), mThDepth(frame.mThDepth), N(frame.N), mvKeys(frame.mvKeys),
				...
				for (size_t j=0, jend=vCell.size(); j<jend; j++){
					...
					if (bCheckLevels){
						if (kpUn.octave<minLevel)
							continue;
						if (maxLevel>=0) if (kpUn.octave>maxLevel)
							continue;
					}
	...
	void Frame::ComputeStereoMatches(){
		...
		for (int iL=0; iL<N; iL++){
			...
			if (bestDist<ORBmatcher::TH_HIGH){
				...
				const int w=5;
				cv::Mat IL=mpORBextractorLeft->mvImagePyramid[kpL.octave].rowRange(scaledvL-w, scaledvL+w+1).colRange(
