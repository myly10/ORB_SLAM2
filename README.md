...
## Modifications
- Able to save key frame images on the fly or in final save trajectory stage. Images will be saved to `keyimages` folder for monocular, and `keyimages/L` and `keyimages/R` for stereo. (controlled by `mbSaveImageOnTheFly` in `System.h`, have to be determined before compiling currently)
- Loads DBoW vocabulary in binary format, which greatly increases the loading speed (more than 100 times for me)
